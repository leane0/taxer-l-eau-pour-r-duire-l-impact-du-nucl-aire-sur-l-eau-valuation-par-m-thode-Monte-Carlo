...
def montecarlo_parameters(n_sim, cnpe_choice, scenario):
    ...
    for i in range(n_sim):
        ...
        if cnpe_choice in ["Tricastin", "Saint-Alban", "Bugey"]:
            ...
            gain_redevance = (
                param["TAUX_REDEVANCE_PREL"][scenario]
                * param["V_PREL_OUVERT"] * 1e4
                + 9.1
...
if __name__ == "__main__":
    ...
    for CNPE in CNPEs:
        ...
        for scenario in scenarios:
            ...
            print(scenario  )
            print(np.mean(data["REDEVANCE"]) / 1e6)
            ...
            NPV_results, ROI_results = montecarlo_simulation(
                n_sim,
                CNPE,
                scenario
            )
            ...
            with open(
                os.path.join(scenario_dir, "monte_carlo_results.pkl"),
                "wb"
            ) as f:
                pickle.dump(data, f)
            continue
            ...
            plt.savefig(os.path.join(scenario_dir,"tornado.png"),
                dpi=300,
                bbox_inches="tight"
            )
            ...
            plt.savefig(
                os.path.join(scenario_dir,"sobol.png"),
                dpi=300,
                bbox_inches="tight"
            )
