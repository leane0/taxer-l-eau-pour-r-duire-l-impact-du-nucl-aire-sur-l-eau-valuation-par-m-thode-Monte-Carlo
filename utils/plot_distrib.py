...
# Pour tracer les fonctions de répartition pour plusieurs centrales à la fois
...
# Récupération des fichiers contenant les résultats
...
# Construction fonction de répartition empirique
...
def plot_cdf(cnpe_group, scenario, variable):
    ...
    for CNPE in cnpe_group:
        ...
        if variable == "NPV":
            ...
            raise ValueError("variable doit être 'NPV' ou 'ROI'" )

        # CDF
        x, F = empirical_cdf(values)
        plt.plot(x,F,label=CNPE )
    ...
    plt.xlabel(xlabel)
    plt.ylabel(f"P({xlabel.split()[0]} ≤ x)")
    plt.title(f"Empirical CDF of {variable} — {scenario}")
    plt.ylim(0, 1)
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    ...
    output_dir = os.path.join( RESULTS_DIR, "CDF")
...
def plot_cdf_nogent( scenarios, variable):
    ...
    for scenario in scenarios:
        ...
        plt.plot( x,F, label=label[scenario])
